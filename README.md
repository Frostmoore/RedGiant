...
- **F18 — With references made coherent, the residual wall is executor capability — now visible and attributable.** *(open hypothesis, pending the PS6 A/B)* Once the compiler side stabilized, roughly half of all remaining failures had one shape: the junior cannot reproduce the exact output formats the qualified tests demand, even when shown the test source verbatim and the failing assertion diff on retry. Every death now has a stage, a cause and a cost — the harness's job is not to make a 2B smarter, but to make its limits legible and fenced; a stronger model dropped into the same scaffold inherits the entire discipline for free.
