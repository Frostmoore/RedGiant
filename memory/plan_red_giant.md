...
### D22 — Ricerca della letteratura PRIMA di costruire o misurare (2026-08-04)

- **Enunciato:** prima di ogni test, ogni implementazione e ogni attività di progetto si verifica se **esiste già ricerca sull'argomento**. Non a posteriori, non "se avanza tempo": prima. La ricerca si fa sul **fenomeno** (es. *"agent falsely claims task completed"*), non sull'etichetta che gli abbiamo dato noi. Se si trova, si legge, si **cita nel piano e nel white paper**, e si decide se il nostro esperimento aggiunge qualcosa — spesso aggiunge, ma nel **regime**, non nel concetto. Se dopo 2-3 ricerche mirate non si trova nulla, si scrive *"non trovato"*, che è evidenza debolissima, mai *"nuovo"*.
- **Perché:** l'audit di novità del 2026-08-04 (`data.md` §7.19) ha cercato **una per una** le sette cose che il progetto credeva proprie e le ha trovate **tutte e sette** già pubblicate — una con lo stesso nome che le avevamo dato indipendentemente (*false success* per la forbice `completed ≠ verified`), e almeno una misurata meglio di come l'abbiamo misurata noi (la decomposizione impalcatura/verifica quantificata in punti percentuali). Ha anche un beneficio diretto oltre al risparmio di tempo: **due volte la ricerca ha cambiato il progetto in meglio** — ha rivelato `--swa-full`, che ha demolito un vincolo attorno a cui era stata progettata un'intera fase (§6.4-quinquies), e ha mostrato che la compattazione a ondate batte lo sfratto continuo.
- **Conseguenze operative:** ogni sottofase nuova nasce con una riga *"letteratura consultata"* prima della riga *"previsione registrata"*; le fonti si registrano in `data.md` accanto alla misura che riguardano, così non si ricerca due volte; il white paper cita chi ha fatto una cosa prima di noi — pubblicare una decomposizione senza citare chi l'ha già pubblicata ci farebbe sembrare disinformati o disonesti.
- **Violazione tipica:** cercare *dopo* aver misurato per "vedere se qualcuno conferma", che è ricerca di conferma e non di priorità; oppure cercare il tema generale ("agenti LLM") invece del meccanismo specifico, non trovare nulla di pertinente, e concludere che il meccanismo è nostro.
