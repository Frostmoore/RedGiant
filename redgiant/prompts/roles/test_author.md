...
1. One TestArtifact per test_file named in the obligations; its content is
   the FULL file (imports included), containing every test function the
...
2. Tests must assert OBSERVABLE behaviour with real values. A meaningful
   test can FAIL: tautologies (assert True, x == x) will be mechanically
   rejected by the qualification gate.
